...
def test_bulk_hours_import_success(
...
def test_bulk_hours_import_upsert(
    authenticated_client: TestClient, test_db, test_user_a
):
...
def test_bulk_hours_import_empty(
    authenticated_client: TestClient, test_db, test_user_a
):
...
def test_import_feeds_yoy_comparison(
    authenticated_client: TestClient, test_db, test_user_a
):
...
def test_imported_expenses_in_yoy(
    authenticated_client: TestClient, test_db, test_user_a
):
...
def test_imported_recurring_expenses_in_overview(
    authenticated_client: TestClient, test_db, test_user_a
):
    ...
    for i, month in enumerate(months):
        assert (
            month["recurring_expenses"] == 2600.0
...
def test_bulk_hours_invalid_year(
    authenticated_client: TestClient, test_db, test_user_a
):
    ...
    for bad_year in [2019, 2101]:
        ...
        assert (
            res.status_code == 422
        ), f"Expected 422 for year={bad_year}, got {res.status_code}"
