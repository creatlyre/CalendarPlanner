...
def _seed_settings(
    test_db, calendar_id, rate_1=100, rate_2=80, rate_3=60, zus=1500, acc=500, year=2026
):
...
def test_get_income_with_settings_defaults(
    authenticated_client: TestClient, test_db, test_user_a
):
...
def test_save_hours_updates_calculations(
    authenticated_client: TestClient, test_db, test_user_a
):
...
def test_hours_validation_negative(authenticated_client: TestClient):
    resp = authenticated_client.put(
        "/api/budget/income/hours", json={"year": 2026, "month": 1, "rate_1_hours": -10}
    )
...
def test_hours_validation_bad_month(authenticated_client: TestClient):
    resp = authenticated_client.put(
        "/api/budget/income/hours", json={"year": 2026, "month": 0, "rate_1_hours": 100}
    )
    assert resp.status_code == 422

    resp = authenticated_client.put(
        "/api/budget/income/hours",
        json={"year": 2026, "month": 13, "rate_1_hours": 100},
    )
    assert resp.status_code == 422
...
def test_hours_rounds_to_half(authenticated_client: TestClient, test_db, test_user_a):
    ...
    resp = authenticated_client.put(
        "/api/budget/income/hours",
        json={"year": 2026, "month": 1, "rate_1_hours": 100.3},
    )
    assert resp.status_code == 200
...
def test_add_earning(authenticated_client: TestClient, test_db, test_user_a):
    resp = authenticated_client.post(
        ...
        json={"year": 2026, "month": 3, "name": "Pensja partnerki", "amount": 3500.00},
    )
    assert resp.status_code == 200
    data = resp.json()["data"]
...
def test_add_earning_appears_in_income(
    authenticated_client: TestClient, test_db, test_user_a
):
    authenticated_client.post(
        "/api/budget/income/earnings",
        json={"year": 2026, "month": 3, "name": "500+", "amount": 800.00},
    )
    resp = authenticated_client.get("/api/budget/income?year=2026")
...
def test_delete_earning(authenticated_client: TestClient, test_db, test_user_a):
    ...
    resp = authenticated_client.post(
        "/api/budget/income/earnings",
        json={"year": 2026, "month": 5, "name": "Bonus", "amount": 1000.00},
    )
...
def test_earning_validation_negative_amount(authenticated_client: TestClient):
    resp = authenticated_client.post(
        "/api/budget/income/earnings",
        json={"year": 2026, "month": 1, "name": "Test", "amount": -100},
    )
    assert resp.status_code == 422
...
def test_add_recurring_earning(authenticated_client: TestClient, test_db, test_user_a):
    ...
    resp = authenticated_client.post(
        "/api/budget/income/earnings",
        json={"year": 2026, "month": 0, "name": "Vale", "amount": 300.00},
    )
    assert resp.status_code == 200
    data = resp.json()["data"]
...
def test_recurring_earning_appears_in_all_months(
    authenticated_client: TestClient, test_db, test_user_a
):
    """Recurring earnings show in every month's recurring_earnings list."""
    authenticated_client.post(
        "/api/budget/income/earnings",
        json={"year": 2026, "month": 0, "name": "Pensja partnerki", "amount": 4000.00},
    )
    resp = authenticated_client.get("/api/budget/income?year=2026")
    data = resp.json()["data"]
...
def test_recurring_earning_does_not_mix_with_monthly(
    authenticated_client: TestClient, test_db, test_user_a
):
    ...
    authenticated_client.post(
        "/api/budget/income/earnings",
        json={"year": 2026, "month": 3, "name": "Bonus", "amount": 1000.00},
    )
    resp = authenticated_client.get("/api/budget/income?year=2026")
    data = resp.json()["data"]
    assert len(data["recurring_earnings"]) == 1
...
def test_delete_recurring_earning(
    authenticated_client: TestClient, test_db, test_user_a
):
    resp = authenticated_client.post(
        "/api/budget/income/earnings",
