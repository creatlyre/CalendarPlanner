...
def test_create_event_then_day_panel_contains_it(authenticated_client):
    ...
    day_response = authenticated_client.get(f"/calendar/day?year={start.year}&month={start.month}&day={start.day}")
...
def test_update_then_delete_flow(authenticated_client):
    ...
    update_response = authenticated_client.put(f"/api/events/{event_id}", json={"title": "Permanent Event"})
    assert update_response.status_code == 200

    month_response = authenticated_client.get(f"/calendar/month?year={now.year}&month={now.month}")
    assert "Permanent Event" in month_response.text

    delete_response = authenticated_client.delete(f"/api/events/{event_id}")
    assert delete_response.status_code == 200

    month_response_after_delete = authenticated_client.get(f"/api/events/month?year={now.year}&month={now.month}")
