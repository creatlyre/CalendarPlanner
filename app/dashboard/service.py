...
class DashboardService:
    ...
    def get_week_preview(
        self, calendar_id: str, user_id: str, days: int = 7
    ) -> list[dict]:
    ...
    def get_top_expense_categories(
        self, calendar_id: str, limit: int = 3
    ) -> list[dict]:
        now = datetime.utcnow()
        try:
            breakdown = self.expense_service.get_category_breakdown(
                calendar_id, now.year
            )
        except Exception:
            return []
    ...
    def get_onetime_expenses(self, calendar_id: str) -> list[dict]:
        ...
        try:
            ...
            return []
        return [
            e for e in data.get("onetime_expenses", []) if e.get("month") == now.month
        ]
