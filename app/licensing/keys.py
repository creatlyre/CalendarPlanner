"""HMAC-based license key generation and validation for Dobry Plan self-hosted."""
...
def generate_license_key(secret: str) -> str:
    ...
    Format: DOBRYPLAN-XXXXXXXX-XXXXXXXX-XXXXXXXX-XXXXXXXX-CCCCCCCC
...
def validate_license_key(key: str, secret: str) -> bool:
    """Validate a Dobry Plan license key against the secret.
    ...
    if not isinstance(key, str):
        return False
    parts = key.split("-")
    if len(parts) != 6 or parts[0] != "DOBRYPLAN":
        return False
    try:
